...
class Kefir(models.Model):
    ...
    fat_content = models.DecimalField(validators=[MinValueValidator(0.0), MaxValueValidator(100.0)], max_digits=3, decimal_places=1, verbose_name="Жирность (%)", default=2.5)
    volume = models.PositiveIntegerField(validators=[MinValueValidator(0)], verbose_name="Объем (мл)", default=500)
    is_lactose_free = models.BooleanField(default=False, verbose_name="Безлактозный")
    shelf_life = models.PositiveIntegerField(validators=[MinValueValidator(0)], verbose_name="Срок годности (дни)", default=7)
    kefir_type = models.ForeignKey(KefirType, on_delete=models.PROTECT, verbose_name="Тип кефира")
    extra_flavor = models.CharField(max_length=100, verbose_name="Дополнительный вкус", blank=True, null=True)
    description = models.TextField(blank=True, verbose_name="Описание")
    image = models.ImageField(upload_to='kefir_images/', verbose_name="Изображение", blank=True, null=True)

    price = models.DecimalField(validators=[MinValueValidator(0.0)], max_digits=6, decimal_places=2, verbose_name="Розничная цена", default=250)
    small_wholesale_price = models.DecimalField(validators=[MinValueValidator(0.0)], max_digits=6, decimal_places=2, verbose_name="Мелкооптовая цена", default=220)
    small_wholesale_min_qty = models.PositiveIntegerField(verbose_name="Мин. кол-во для мелкого опта", default=10)
    large_wholesale_price = models.DecimalField(validators=[MinValueValidator(0.0)], max_digits=6, decimal_places=2, verbose_name="Крупнооптовая цена", default=200)
    large_wholesale_min_qty = models.PositiveIntegerField(verbose_name="Мин. кол-во для крупного опта", default=50)

    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Создатель")
    
    def save(self, *args, **kwargs):
        # удаляем user из kwargs, чтобы не передавать его в родительский save()
